...
myObject.someMethod(); //{someMethod : function ()}
...
user.foo(); // Simple function call 
            // false
let fun1 = user.foo1;
fun1(); // true because user.fool1 is a function refrence and function refrence is not bind the object so `this`  is not find in the  function body , so it is bubble out oof the  function scope and search the `this` in pesent in window so window is a boject(host) so it is equal to this 
user.foo1(); // false
...
outerFn(); // Refrence Error : innerItem is not defined 
